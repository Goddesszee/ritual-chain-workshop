...
function offsetNow(hours: number): string {
...
export function CreateBountyForm({ onCreated }: { onCreated?: (bountyId: bigint) => void }) {
  ...
  const [submissionDeadline, setSubmissionDeadline] = useState(offsetNow(1));
  const [revealDeadline,   setRevealDeadline]   = useState(offsetNow(2));
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const subTs = new Date(submissionDeadline).getTime();
    const revTs = new Date(revealDeadline).getTime();
    if (subTs <= Date.now()) { window.alert("Submission deadline must be in the future."); return; }
    const subDeadlineBig = BigInt(Math.floor(subTs / 1000));
    const revDeadlineBig = BigInt(Math.floor(revTs / 1000));
    ...
    try {
      await tx.run({
        ...
        args: [title.trim(), rubric.trim(), subDeadlineBig, revDeadlineBig],
        value,
  ...
  return (
    <Card>
      <CardHeader
        ...
        subtitle="Fund a reward and define two deadlines: one for committing, one for revealing."
      />
      <CardBody>
        ...
        <form onSubmit={handleSubmit} className="mt-3 space-y-3">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <Field label="Submission deadline" hint="Commit phase closes at this time.">
              <Input type="datetime-local" value={submissionDeadline} onChange={(e) => setSubmissionDeadline(e.target.value)} />
            </Field>
            <Field label="Reveal deadline" hint="Reveal phase closes at this time.">
